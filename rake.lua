...
local toggle={esp=true,hud=true,distance=true,distancefade=false,menu=true,watermark=true,barrgb=true,roof=true,distanceunit="meters",distanceposition="below",poweractivity=true,scrapstyle="points",supplylabel=true,supplyitems=true,ringenabled=true,ringshape="circle",ringsize=1,ringspin=false,ringspinspeed=1,rgbdirection="left",powerformat="percent",powerdecimal=true,timerformat="clock",timerwarning=15,hudfontindex=1,teleportcooldown=true,cooldownseconds=30,cooldownuntil=0,cooldownremaining=0,teleporthistory={},ppms=true,ppmsstyle="voltmeter",ppmssquares=5,rakename=true,rakehealth=true,rakenamevalue="rake",rakenamey=0,rakehealthy=0,rakehealthformat="bar",rakebarwidth=70,rakenamecapture=false,hudelements={timer=true,target=true,scrap=true,power=true}}
toggle.hudbar=true;toggle.hudlabels=true;toggle.distanceminimum=true;toggle.distancemin=20;toggle.healthbased=false;toggle.ppmslevel=0;toggle.poweravailable=true;toggle.ppmspowerblocked=false
...
toggle.themestyles={background={labelcolor=themes[themeindex].bg,rgb=false},topbar={labelcolor=themes[themeindex].top,rgb=false},border={labelcolor=themes[themeindex].select,rgb=false},text={labelcolor=themes[themeindex].text,rgb=false}}
local menustate={w=390,h=470,watermarkw=100,watermarkh=27,x=24,y=math.floor((cam.ViewportSize.Y-470)/2),tab=1,minimized=false,maxitems=64,scroll={},scrolltarget={},scrollmax={},itemkinds={}}
local tabnames={"main","skins","misc"}
...
local menutitle=newtext("rake's saint v4 ["..(keynames[keybinds.menu]or tostring(keybinds.menu)).."]",Color3.fromHex("#ffffff"),false,false)
menutitle.Size=13
...
for _,d in ipairs({picker.bg,picker.panel})do setz(d,220)end
for _,d in ipairs({picker.border,picker.innerborder,picker.preview,picker.previewborder,picker.squareborder,picker.hueborder,picker.rgbbg,picker.rgbborder,picker.rgbmark,picker.rgbmarkborder,picker.donebg,picker.doneborder,picker.hexbg,picker.hexborder})do setz(d,222)end
...
local function currentitems()
    if menustate.tab==1 then
        local items={
            ...
            section("scraps",1),{id="scraps",kind="toggle",label="scraps esp",on=espgroups.scraps,col=1},{id="scrapstyleselect",kind="dropdown",label="tier style",value=toggle.scrapstyle,col=1},
            section("distance",1),{id="distance",kind="toggle",label="distance",on=toggle.distance,col=1},{id="distanceminimum",kind="toggle",label="minimum distance",on=toggle.distanceminimum,col=1},{id="distancemin",kind="slider",label="show distance after",value=toggle.distancemin,min=0,max=100,display=tostring(toggle.distancemin).."m",col=1},{id="distancepositionselect",kind="dropdown",label="label position",value=toggle.distanceposition,col=1},
            ...
            section("timer",1),{id="hudtimer",kind="toggle",label="timer counter",on=toggle.hudelements.timer,col=1},{id="timerformatselect",kind="dropdown",label="timer style",value=toggle.timerformat,col=1},{id="timerwarning",kind="slider",label="timer warning",value=toggle.timerwarning,min=10,max=45,display=tostring(toggle.timerwarning).."s",col=1},
...
local function menupos()
    ...
    for i=1,#tabnames do local x=math.floor(navx+(i-1)*tabw+0.5);local right=math.floor(navx+i*tabw+0.5);local w=right-x;local linew=#tabnames[i]*7+10;tabbg[i].Position=Vector2.new(x+math.floor((w-linew)/2),navy+navh-1);tabbg[i].Size=Vector2.new(linew,1);tabborder[i].Position=tabbg[i].Position;tabborder[i].Size=tabbg[i].Size;tabtext[i].Position=Vector2.new(x+math.floor(w/2),navy+8)end
    ...
    for i=1,#menuitems do
        local item=menuitems[i];local col=item.col or 1;local x=left+(col-1)*(w+gap);local y=ys[col];local h=toggle.rowheight(item);local panelh=groupheights[i]or h
        ...
        local overlap=math.min(y+renderh,clipbottom)-math.max(y,cliptop);local visible=overlap>0;local fade=visible and clamp(math.min(y+renderh-cliptop,clipbottom-y)/14,0,1)or 0
        local textfade=visible and math.min(scroll>0 and clamp((texty-cliptop)/12,0,1)or 1,scroll<menustate.scrollmax[menustate.tab]and clamp((clipbottom-texty-10)/12,0,1)or 1)or 0;local valuefade=item.stacked and visible and math.min(scroll>0 and clamp((valuey-cliptop)/12,0,1)or 1,scroll<menustate.scrollmax[menustate.tab]and clamp((clipbottom-valuey-10)/12,0,1)or 1)or textfade
        local liney=y+15;local linefade=item.kind=="section"and visible and clamp(math.min(liney-cliptop,clipbottom-liney)/10,0,1)or fade;local sliderfade=item.kind=="slider"and visible and clamp(math.min(y+17-cliptop,clipbottom-(y+32))/8,0,1)or fade
        ...
        local ratio=item.kind=="slider"and clamp((item.value-item.min)/(item.max-item.min),0,1)or 0
...
local function dropdownvalues()
    ...
    elseif dropdownkind=="scrapstyle"then return {"both","tiers","points"}
...
local function pickerupdate(visible)
    ...
    picker.donebg.Position=Vector2.new(px+178,fy);picker.donebg.Size=Vector2.new(88,27);picker.donebg.Color=color(inside(mouse.X,mouse.Y,px+178,fy,88,27)and"hover"or"top");picker.donebg.Transparency=1;picker.doneborder.Position=picker.donebg.Position;picker.doneborder.Size=picker.donebg.Size;picker.doneborder.Color=color("select");picker.doneborder.Transparency=1;picker.donetext.Position=Vector2.new(px+222,fy+12);picker.donetext.Color=color("text")
...
local function menuobjects(visible)
    ...
    menubg.Visible=visible;menutop.Visible=visible;menuchrome.border.Visible=visible;menutitle.Visible=visible;menuclose.Visible=visible;menuside.Visible=expanded;menuchrome.content.Visible=false;menuchrome.divider.Visible=false
...
local function menuupdate()
    local title="rake's saint v4 ["..(keynames[keybinds.menu]or tostring(keybinds.menu)).."]";if menutitle.Text~=title then menutitle.Text=title;menustate.watermarkw=math.min(menustate.w,math.max(100,#title*7+28))end
    menupos();local mx,my=mouse.X,mouse.Y;local menuoutline=themes[themeindex].light~=true
    menubg.Color=color("bg");menutop.Color=color("top");menuside.Color=color("side");menutitle.Color=color("text");menutitle.Outline=menuoutline;menuclose.Color=color("muted");menuclose.Outline=menuoutline;menuchrome.border.Color=themes.borderblack;menuchrome.columnborders[1].Color=color("muted");menuchrome.columnborders[2].Color=themes.borderblack;menuchrome.content.Color=color("select");menuchrome.divider.Color=color("select");menuchrome.scrolltrack.Color=color("bg");menuchrome.scrollthumb.Color=color("accent");menuchrome.scrollborder.Color=color("select");menubg.Transparency=guiopacity;menutop.Transparency=guiopacity;menuside.Transparency=guiopacity;menuchrome.border.Transparency=guiopacity;menuchrome.columnborders[1].Transparency=0.85*guiopacity;menuchrome.columnborders[2].Transparency=guiopacity;menuchrome.content.Transparency=guiopacity;menuchrome.divider.Transparency=0.7*guiopacity;menuchrome.scrolltrack.Transparency=0.75*guiopacity;menuchrome.scrollthumb.Transparency=guiopacity;menuchrome.scrollborder.Transparency=0.8*guiopacity;menuclose.Text=menustate.minimized and"+"or"-"
    local navw=menustate.w-8;local tabw=navw/#tabnames
    for i=1,#tabnames do local tx=menustate.x+4+(i-1)*tabw;local hover=inside(mx,my,tx,menustate.y+29,tabw,18);tabbg[i].Color=color("accent");tabbg[i].Transparency=guiopacity;tabtext[i].Color=color(i==menustate.tab and"accent"or hover and"accent"or"text");tabtext[i].Outline=menuoutline end
    for i=1,#menuitems do
        local item=menuitems[i];local l=itemlayouts[i];if l.visible then local fade=l.fade or 1;local borderfade=item.kind=="section"and(l.linefade or 0)or item.kind=="slider"and(l.sliderfade or 0)or fade;local hover=l.visible and inside(mx,my,l.x,l.hittop,l.w,l.hitbottom-l.hittop);local shown=toggle.itemshown(item);local strong=item.kind=="dropdown"or item.kind=="action"or item.kind=="bind"or item.kind=="text";itembg[i].Color=color(item.kind=="section"and"top"or strong and(hover and"select"or"top")or hover and"hover"or"card");itembg[i].Transparency=(item.kind=="section"and 0.72 or strong and(hover and 0.96 or item.kind=="action"and 0.82 or 0.58)or hover and 0.38 or 0.13)*guiopacity*fade;itemborder[i].Color=themes.borderblack;itemborder[i].Transparency=guiopacity*borderfade;markborder[i].Color=color("muted");markborder[i].Transparency=0.9*guiopacity*borderfade;trackborder[i].Color=themes.borderblack;trackborder[i].Transparency=guiopacity*borderfade;itemlabel[i].Text=item.label;itemlabel[i].Color=color("text");itemlabel[i].Transparency=l.textfade;itemlabel[i].Outline=menuoutline;itemvalue[i].Text=shown;itemvalue[i].Color=color(item.stacked and"text"or"accent");itemvalue[i].Transparency=l.valuefade;itemvalue[i].Outline=menuoutline;itemline[i].Color=color("accent");itemline[i].Transparency=0.85*guiopacity*borderfade;itemtrack[i].Color=color("bg");itemtrack[i].Transparency=item.kind=="slider"and borderfade or fade;itemfill[i].Color=color("accent");itemfill[i].Transparency=item.kind=="section"and borderfade or item.kind=="slider"and borderfade or fade
        if item.kind=="toggle"then itemmark[i].Color=color("accent");itemmark[i].Transparency=item.on and fade or 0
        elseif item.kind=="color"then
            local cfg=entrycfg(item.index);itemmark[i].Color=cfg and cfg.labelcolor or color("accent");itemmark[i].Transparency=fade;itemvalue[i].Text=cfg and cfg.rgb and"RGB"or""
            local p=itemmark[i].Position;local size=itemmark[i].Size
            if cfg and cfg.rgb then
                local segs=getitemrgb(i);local innerw=math.max(0,size.X)
                for s=1,itemrgbcount do local left=math.floor((s-1)*innerw/itemrgbcount+0.5);local right=math.floor(s*innerw/itemrgbcount+0.5);segs[s].Position=Vector2.new(p.X+left,p.Y);segs[s].Size=Vector2.new(math.max(0,right-left),math.max(0,size.Y));segs[s].Color=rgb((s-1)/itemrgbcount*rgbspread);segs[s].Transparency=fade end
            end
        end
        local frame=sectionframes[i];if frame and item.kind=="section"then local bottomcenter=l.y+l.panelh-2;local bottomfade=clamp(math.min(bottomcenter-l.cliptop,l.clipbottom-bottomcenter)/10,0,1);frame.topl.Color=color("accent");frame.topr.Color=color("accent");frame.topl.Transparency=guiopacity*(l.linefade or 0);frame.topr.Transparency=guiopacity*(l.linefade or 0);frame.left.Color=color("select");frame.right.Color=color("select");frame.bottom.Color=color("select");frame.left.Transparency=guiopacity*fade;frame.right.Transparency=guiopacity*fade;frame.bottom.Transparency=guiopacity*bottomfade end
    end end
...
local function powerpos()
    ...
    for i=1,#powerlines do if powerlines[i].Visible then n=n+1 end end
    p.h=36+n*17;p.x=clamp(p.x,0,math.max(0,cam.ViewportSize.X-p.w));p.y=clamp(p.y,0,math.max(0,cam.ViewportSize.Y-p.h));local on=toggle.hud and toggle.poweractivity and n>0
    n=0;for i=1,#powerlines do local line=powerlines[i];if line.Visible then n=n+1;line.Position=Vector2.new(p.x+10,p.y+29+(n-1)*17);line.Color=color("text");line.Transparency=guiopacity end end
    p.outer.Position=Vector2.new(p.x,p.y);p.outer.Size=Vector2.new(p.w,p.h);p.outer.Color=themes.borderblack;p.outer.Transparency=guiopacity;p.outer.Visible=on
    p.middle.Position=Vector2.new(p.x+1,p.y+1);p.middle.Size=Vector2.new(p.w-2,p.h-2);p.middle.Color=color("muted");p.middle.Transparency=0.85*guiopacity;p.middle.Visible=on
    p.inner.Position=Vector2.new(p.x+2,p.y+2);p.inner.Size=Vector2.new(p.w-4,p.h-4);p.inner.Color=themes.borderblack;p.inner.Transparency=guiopacity;p.inner.Visible=on
    p.bg.Position=Vector2.new(p.x+3,p.y+3);p.bg.Size=Vector2.new(p.w-6,p.h-6);p.bg.Color=color("bg");p.bg.Transparency=guiopacity;p.bg.Visible=on
    p.top.Position=Vector2.new(p.x+3,p.y+3);p.top.Size=Vector2.new(p.w-6,21);p.top.Color=color("top");p.top.Transparency=guiopacity;p.top.Visible=on
    p.accent.Position=Vector2.new(p.x+3,p.y+3);p.accent.Size=Vector2.new(p.w-6,2);p.accent.Color=toggle.barrgb and rgb(0)or color("accent");p.accent.Transparency=guiopacity;p.accent.Visible=on
    p.divider.From=Vector2.new(p.x+3,p.y+24);p.divider.To=Vector2.new(p.x+p.w-3,p.y+24);p.divider.Color=color("select");p.divider.Transparency=0.7*guiopacity;p.divider.Visible=on
    powerlabel.Position=Vector2.new(p.x+10,p.y+9);powerlabel.Color=Color3.fromHex("#ffffff");powerlabel.Visible=on and toggle.hudlabels;powerlabel.Transparency=guiopacity
end
...
local function powerhud()
    ...
    for i=1,#powercfg do
        ...
        if active then activecells=activecells+(entry.cells or 0);activecount=activecount+1 end;powerlines[i].Visible=toggle.hud and toggle.poweractivity and active or false;if powerlines[i].Visible then any=true end
    end
...
local function drawrgb()
    ...
    if toggle.hud and toggle.hudbar and(toggle.hudcount or 0)>0 then for i=1,#rgbline do local d=rgbline[i];d.Color=toggle.barrgb and Color3.fromHSV((phase+(i-1)/barseg*rgbspread)%1,0.68,1)or color("accent");d.Visible=true end else for i=1,#rgbline do rgbline[i].Visible=false end end
    if toggle.menu then for i=1,#menurgb do local d=menurgb[i];d.Color=toggle.barrgb and Color3.fromHSV((phase+(i-1)/barseg*rgbspread)%1,0.68,1)or color("accent");d.Visible=true end end
    toggle.powerpanel.accent.Color=toggle.barrgb and Color3.fromHSV(phase,0.68,1)or color("accent")
    for _,segs in pairs(itemrgb)do for i=1,itemrgbcount do if segs[i].Visible then segs[i].Color=Color3.fromHSV((phase+(i-1)/itemrgbcount*rgbspread)%1,0.68,1)end end end
end
...
toggle.setscrapstyle=function(value,quiet)
    toggle.scrapstyle=value=="points"and"points"or value=="tiers"and"tiers"or"both";local points={12,15,19,23,27}
    for i=1,5 do local label=toggle.scrapstyle=="tiers"and("scrap "..i)or toggle.scrapstyle=="points"and("scrap [+"..points[i].."]")or("scrap "..i.." [+"..points[i].."]");local cfg=espcfg["Scrap"..tostring(i)];if cfg then cfg.text=label end end;for i=1,#tracked do local rec=tracked[i];if rec.cfgname and string.match(rec.cfgname,"^Scrap%d$")and rec.name then rec.name.Text=rec.cfg.text end end
...
local function configdata()
    ...
    for _,id in ipairs({"themebg","themetop","themeborder","themetext"})do local cfg=entrycfg(id);local c=cfg.labelcolor;themecolors[id]={r=channel(c.R),g=channel(c.G),b=channel(c.B)}end
...
local function loadconfig(quiet)
    ...
    local savedpreset=data.preset or data.theme;local removedlight=false
    if data.preset_schema~=2 and type(savedpreset)=="number"then removedlight=savedpreset==2 or savedpreset==6 or savedpreset==10;savedpreset=({1,1,2,3,4,4,5,6,7,7,8,9})[math.floor(savedpreset)]or 5 end
    if type(data.preset_name)=="string"then for i=1,#themes do if themes[i].name==data.preset_name then savedpreset=i;break end end end
    if type(savedpreset)=="number"then setthemeindex(clamp(savedpreset,1,#themes),true)end
    local legacygamesense=data.theme_schema~=2 and savedpreset==5
    ...
    if data.rake_health_format=="value"or data.rake_health_format=="bar"then toggle.setrakehealthformat(data.rake_health_format,true)else toggle.setrakehealthformat("bar",true)end
    ...
    toggle.distancefade=false;if data.scrap_style=="both"or data.scrap_style=="tiers"or data.scrap_style=="points"then toggle.setscrapstyle(data.scrap_style,true)else toggle.setscrapstyle("both",true)end
...
toggle.resettoggles=function(quiet)
    setesp(true,true);sethud(true,true);for _,id in ipairs({"timer","target","scrap","power"})do toggle.sethudelement(id,true,true)end;toggle.watermark=true;toggle.hudbar=true;toggle.hudlabels=true;toggle.distanceminimum=true;toggle.distancemin=20;toggle.healthbased=false;toggle.setpoweractivity(true,true);toggle.cooldownuntil=0;toggle.cooldownremaining=0;toggle.teleporthistory={};toggle.setteleportcooldown(true,true);toggle.setcooldownseconds(30,true);toggle.setppms(true,true);toggle.setppmsstyle("voltmeter",true);toggle.setppmssquares(5,true);toggle.setroof(true,true);toggle.setrakename(true,true);toggle.setrakehealth(true,true);toggle.rakenamevalue="rake";toggle.rakedraw.name.Text="rake";toggle.setrakenamey(0,true);toggle.setrakehealthy(0,true);toggle.setrakehealthformat("bar",true);toggle.setrakebarwidth(70,true);setbarrgb(true,true);toggle.setrgbdirection("left",true);toggle.setrgbspeed(0.6,true);setdistance(true,true);toggle.distancefade=false;toggle.setunit("meters",true);toggle.setdistanceposition("below",true);toggle.setscrapstyle("points",true);toggle.setsupplylabel(true,true);toggle.setsupplyitems(true,true);toggle.setringenabled(true,true);toggle.setringshape("circle",true);toggle.setringfade(40,true);toggle.setringsize(1,true);toggle.setringspin(false,true);toggle.setringspinspeed(1,true);toggle.setpowerformat("percent",true);toggle.setpowerdecimal(true,true);toggle.settimerformat("clock",true);toggle.settimerwarning(15,true)
...
local function clickmenu(mx,my)
    ...
    local navw=menustate.w-8;local tabw=navw/#tabnames
    for i=1,#tabnames do
        if inside(mx,my,menustate.x+4+(i-1)*tabw,menustate.y+29,tabw,18)then if configcapture then toggle.finishconfiginput(false)end;if toggle.rakenamecapture then toggle.finishrakename(false)end;menustate.tab=i;capture=nil;pickerentry=nil;picker.hexactive=false;dropdownkind=nil;menuupdate();return end
    end
