...
local menustate={w=392,h=470,watermarkw=100,watermarkh=27,x=24,y=math.floor((cam.ViewportSize.Y-470)/2),tab=1,minimized=false,minimizeanim=0,menuanim=0,contentfade=1,maxitems=96,dragsensitivity=1.6,scroll={},scrolltarget={},scrollmax={},itemkinds={},hover={},toggleanim={},slideranim={},tabfade=1,tabslide=0,indicatorx=nil,indicatorw=nil,itemsdirty=true}
...
local function currentitems()
    ...
    if menustate.tab==1 then
        ...
        local items={
            ...
            section("information",2),{id="worldscraps",kind="info",label="scraps in world",value=tostring(toggle.worldinfo.scraps),col=2},{id="worldpoints",kind="info",label="total points",value=tostring(toggle.worldinfo.points),subtle=true,col=2},{id="worldflare",kind="info",label="flare spawned",value=toggle.worldinfo.flare and"yes"or"no",col=2},{id="worldtraps",kind="info",label="traps in world",value=tostring(toggle.worldinfo.traps),col=2},{id="worldcrates",kind="info",label="crates in world",value=tostring(toggle.worldinfo.crates),col=2},
            ...
            section("scraps esp",1),{id="scrapstyleselect",kind="dropdown",label="scrap style",value=toggle.scrapstyle,col=1},{id="itemScrap1",kind="toggle",label="scrap 1",on=espgroups.items.Scrap1,itemkey="Scrap1",colorindex=2,col=1},{id="itemScrap2",kind="toggle",label="scrap 2",on=espgroups.items.Scrap2,itemkey="Scrap2",colorindex=3,col=1},{id="itemScrap3",kind="toggle",label="scrap 3",on=espgroups.items.Scrap3,itemkey="Scrap3",colorindex=4,col=1},{id="itemScrap4",kind="toggle",label="scrap 4",on=espgroups.items.Scrap4,itemkey="Scrap4",colorindex=5,col=1},{id="itemScrap5",kind="toggle",label="scrap 5",on=espgroups.items.Scrap5,itemkey="Scrap5",colorindex=6,col=1},
...
local function powerpos()
    ...
    local showalways=toggle.poweractivitymode=="always";local hasactivity=n>0;local empty=toggle.poweractivity and showalways and not hasactivity;local rows=toggle.poweractivity and(empty and 1 or n)or 0;local volt=toggle.ppms and not toggle.ppmspowerblocked and(hasactivity or showalways);local shown=math.min(toggle.ppmslevel or 0,5);local contentw=toggle.textwidth(powerlabel);if empty then contentw=math.max(contentw,toggle.textwidth(toggle.powerempty))end;for i=1,#powerlines do if p.lineactive[i]then contentw=math.max(contentw,toggle.textwidth(powerlines[i]))end end;if volt and shown>0 then contentw=math.max(contentw,shown*12+(shown-1)*3)end;p.w=math.max(106,contentw+26);p.h=40+rows*17+(volt and 23 or 0);p.x=clamp(p.x,0,math.max(0,cam.ViewportSize.X-p.w));p.y=clamp(p.y,0,math.max(0,cam.ViewportSize.Y-p.h));local activityon=toggle.poweractivity and(hasactivity or showalways);local on=toggle.hud and(activityon or volt);p.anim=toggle.ease(p.anim,on and 1 or 0,on and 0.26 or 0.3);local drawon=p.anim>0.01;local drawy=p.y+math.floor((1-p.anim)*8+0.5);local alpha=guiopacity*p.anim;local contentalpha=p.anim
