...
for _,d in ipairs({picker.border,picker.middleborder,picker.innerborder,picker.panelborder,picker.preview,picker.previewborder,picker.squareborder,picker.hueborder,picker.rgbbg,picker.rgbborder,picker.rgbmark,picker.rgbmarkborder,picker.donebg,picker.doneborder,picker.hexbg,picker.hexborder})do setz(d,222)end
...
local function pickerupdate(visible)
    ...
    local alpha=picker.anim;picker.border.Position=Vector2.new(px,py);picker.border.Size=Vector2.new(pw,ph);picker.border.Color=themes.borderblack;picker.border.Transparency=alpha;picker.middleborder.Position=Vector2.new(px+1,py+1);picker.middleborder.Size=Vector2.new(pw-2,ph-2);picker.middleborder.Color=color("muted");picker.middleborder.Transparency=0.85*alpha;picker.innerborder.Position=Vector2.new(px+2,py+2);picker.innerborder.Size=Vector2.new(pw-4,ph-4);picker.innerborder.Color=themes.borderblack;picker.innerborder.Transparency=alpha
    ...
    picker.rgbborder.Position=Vector2.new(px+98,fy);picker.rgbborder.Size=Vector2.new(72,27);picker.rgbborder.Color=themes.borderblack;picker.rgbborder.Transparency=alpha;picker.rgbbg.Position=Vector2.new(px+100,fy+2);picker.rgbbg.Size=Vector2.new(68,23);picker.rgbbg.Color=color("top");picker.rgbbg.Transparency=alpha;picker.rgbmarkborder.Position=Vector2.new(px+106,fy+8);picker.rgbmarkborder.Size=Vector2.new(11,11);picker.rgbmarkborder.Color=themes.borderblack;picker.rgbmarkborder.Transparency=alpha;picker.rgbmark.Position=Vector2.new(px+108,fy+10);picker.rgbmark.Size=Vector2.new(7,7);picker.rgbmark.Color=toggle.accentvisual();picker.rgbmark.Transparency=cfg.rgb and alpha or 0;picker.rgbtext.Position=Vector2.new(px+124,fy+6);picker.rgbtext.Color=color("text");picker.rgbtext.Transparency=alpha
...
local function menuupdate()
    ...
    for i=1,#menuitems do
        local item=menuitems[i];local l=itemlayouts[i];if l.visible then local fade=l.fade or 1;local borderfade=item.kind=="section"and(l.linefade or 0)or item.kind=="slider"and(l.sliderfade or 0)or fade;local hover=l.visible and item.kind~="section"and inside(mx,my,l.x,l.hittop,l.w,l.hitbottom-l.hittop);local shown=toggle.itemshown(item);local strong=item.kind=="dropdown"or item.kind=="action"or item.kind=="bind"or item.kind=="text";local animkey=tostring(menustate.tab)..":"..tostring(item.id or item.label or i);menustate.hover[animkey]=toggle.ease(menustate.hover[animkey],hover and 1 or 0,0.2);local basecolor=item.kind=="section"and"top"or strong and"top"or"card";local hovercolor=strong and"select"or"hover";itembg[i].Color=toggle.colormix(color(basecolor),color(hovercolor),menustate.hover[animkey]);itembg[i].Transparency=(item.kind=="section"and 0.72 or strong and(0.58+0.38*menustate.hover[animkey])or 0.13+0.25*menustate.hover[animkey])*guiopacity*fade;itemborder[i].Color=themes.borderblack;itemborder[i].Transparency=guiopacity*borderfade;markborder[i].Color=color("muted");markborder[i].Transparency=0.9*guiopacity*borderfade;trackborder[i].Color=themes.borderblack;trackborder[i].Transparency=guiopacity*borderfade;itemlabel[i].Text=item.label;itemlabel[i].Color=color("text");itemlabel[i].Transparency=l.textfade;itemlabel[i].Outline=menuoutline;itemvalue[i].Text=shown;itemvalue[i].Color=color(item.stacked and"text"or"accent");itemvalue[i].Transparency=l.valuefade;itemvalue[i].Outline=menuoutline;itemline[i].Color=toggle.accentvisual();itemline[i].Transparency=0.85*guiopacity*borderfade;itemtrack[i].Color=color("bg");itemtrack[i].Transparency=item.kind=="slider"and borderfade or fade;itemfill[i].Color=toggle.accentvisual();itemfill[i].Transparency=item.kind=="section"and borderfade or item.kind=="slider"and borderfade or fade
...
local function powerpos()
    ...
    p.middle.Position=Vector2.new(p.x+1,drawy+1);p.middle.Size=Vector2.new(p.w-2,p.h-2);p.middle.Color=color("muted");p.middle.Transparency=0.85*alpha;p.middle.Visible=drawon
