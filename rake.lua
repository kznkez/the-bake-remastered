...
local function currentitems()
    if menustate.tab==1 then
        local items={
            ...
            section("hud preferences",1),{id="hudbar",kind="toggle",label="accent bar",on=toggle.hudbar,col=1},{id="hudlabels",kind="toggle",label="hud labels",on=toggle.hudlabels,col=1},
            ...
            section("rake",2),{id="rake",kind="toggle",label="rake esp",on=espgroups.rake,col=2},{id="rakename",kind="toggle",label="rake name",on=toggle.rakename,col=2},{id="rakehealth",kind="toggle",label="rake health",on=toggle.rakehealth,col=2},{id="rakedistance",kind="toggle",label="rake distance",on=toggle.rakedistance,col=2},{id="rakenameinput",kind="text",label="custom name",value=toggle.rakenamecapture and toggle.rakenamevalue.."_"or toggle.rakenamevalue,col=2},{id="healthbased",kind="toggle",label="health-based color",on=toggle.healthbased,col=2},{id="rakehealthformatselect",kind="dropdown",label="health style",value=toggle.rakehealthformat,col=2},{id="rakenamey",kind="slider",label="name Y offset",value=toggle.rakenamey,min=-100,max=100,display=tostring(toggle.rakenamey).."px",col=2},{id="rakehealthy",kind="slider",label="health Y offset",value=toggle.rakehealthy,min=-100,max=100,display=tostring(toggle.rakehealthy).."px",col=2},{id="rakebarwidth",kind="slider",label="health bar width",value=toggle.rakebarwidth,min=30,max=200,display=tostring(toggle.rakebarwidth).."px",col=2},
            section("power hud",2),{id="hudpower",kind="toggle",label="power left",on=toggle.hudelements.power,col=2},{id="powerdecimal",kind="toggle",label="decimal value",on=toggle.powerdecimal,col=2},
            section("power usage",2),{id="poweractivity",kind="toggle",label="activity gui",on=toggle.poweractivity,col=2},{id="ppms",kind="toggle",label="hud voltmeter",on=toggle.ppms and not toggle.ppmspowerblocked,col=2},
...
toggle.drawrake=function(viewer)
    ...
    d.distance.Text=tostring(math.floor(meters)).."m";d.distance.Position=Vector2.new(screen.X,screen.Y+espfontsize+10);d.distance.Color=toggle.distancestyle.rgb and rgb(0)or toggle.distancestyle.labelcolor;d.distance.Transparency=1;d.distance.Size=espfontsize;d.distance.Font=font;d.distance.Visible=toggle.rakedistance
end
...
local function loadconfig(quiet)
    ...
    toggle.writelastconfig(configname);powerpos();menupos();menuupdate();if not quiet then bindlog("loaded config")end;return true
end
