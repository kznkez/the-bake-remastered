...
local defaultbinds={menu=0x43,esp=0,hud=0,scrap=0,flare=0}
...
local menustate={w=392,h=470,watermarkw=100,watermarkh=27,x=24,y=math.floor((cam.ViewportSize.Y-470)/2),tab=1,minimized=false,minimizeanim=0,contentfade=1,maxitems=72,scroll={},scrolltarget={},scrollmax={},itemkinds={},hover={},toggleanim={},slideranim={},tabfade=1,tabslide=0,indicatorx=nil,indicatorw=nil,itemsdirty=true}
...
toggle.ensurepicker=function()
    ...
    picker.gradient=toggle.makegradient(48,221);for y=1,picker.rows do for x=1,picker.cols do picker.grid[#picker.grid+1]=setz(newsquare(Color3.fromHex("#ffffff"),1),223)end end;for i=1,picker.huesteps do picker.hue[i]=setz(newsquare(Color3.fromHSV((i-1)/picker.huesteps,1,1),1),223)end;picker.ready=true
end
...
local function currentitems()
    if menustate.tab==1 then
        local items={
            ...
            section("client",2),{id="noJumpCooldown",kind="toggle",label="no jump cooldown",on=toggle.client.noJumpCooldown,info="may briefly lag when first enabled while game data is scanned",warning=true,unstable=true,col=2},{id="infiniteStamina",kind="toggle",label="infinite stamina",on=toggle.client.infiniteStamina,info="may briefly lag when first enabled while game data is scanned",warning=true,unstable=true,col=2},{id="noFall",kind="toggle",label="no fall damage",on=toggle.client.noFall,col=2},
            ...
            section("location esp",2),{id="roof",kind="toggle",label="roof HP",on=toggle.roof,col=2}
        }
    ...
    elseif menustate.tab==3 then
        ...
        return {
            ...
            section("keybinds",2),{id="bindmenu",kind="bind",bind="menu",label=bindlabels.menu,value=capture=="menu"and"press a key..."or"["..toggle.bindname("menu").."]",col=2},{id="bindesp",kind="bind",bind="esp",label=bindlabels.esp,value=capture=="esp"and"press a key..."or"["..toggle.bindname("esp").."]",col=2},{id="bindhud",kind="bind",bind="hud",label=bindlabels.hud,value=capture=="hud"and"press a key..."or"["..toggle.bindname("hud").."]",col=2},{id="bindscrap",kind="bind",bind="scrap",label=bindlabels.scrap,value=capture=="scrap"and"press a key..."or"["..toggle.bindname("scrap").."]",col=2},{id="bindflare",kind="bind",bind="flare",label=bindlabels.flare,value=capture=="flare"and"press a key..."or"["..toggle.bindname("flare").."]",col=2},
...
local function displaysize()local phase=clamp(((menustate.minimizeanim or 0)-0.25)/0.75,0,1);return math.floor(menustate.w+(menustate.watermarkw-menustate.w)*phase+0.5),math.floor(menustate.h+(menustate.watermarkh-menustate.h)*phase+0.5)end
...
local function menupos()
    ...
    menutitle.Position=Vector2.new(menustate.x+8,menustate.y+8);menuclose.Position=Vector2.new(menustate.x+displayw-12,menustate.y+8)
    ...
    if menustate.minimized and(menustate.contentfade or 0)<=0.001 then menuitems={};itemlayouts={} elseif menustate.itemsdirty or #menuitems==0 then menuitems=currentitems();menustate.itemsdirty=false;itemlayouts={}else itemlayouts={}end;toggle.menuinfo.layouts={}
    ...
    for i=1,#menuitems do
        ...
        local texty=y+(item.kind=="section"and 8 or item.kind=="action"and 12 or item.kind=="dropdown"and 7 or 4);local valuey=item.kind=="text"and item.stacked and y+27 or item.stacked and y+30 or texty;local renderh=item.kind=="section"and panelh or h
    ...
    local barinset=math.max(3,toggle.borderradius-1);toggle.layoutgradient(menurgb,menustate.x+barinset,menustate.y+3,math.max(1,displayw-barinset*2))
end
...
local function menuupdate(animateonly)
    ...
    if contentfade<0.999 then for i=1,#menuitems do for _,d in ipairs({itembg[i],itemborder[i],itemlabel[i],toggle.menuinfo.items[i],itemvalue[i],itemmark[i],markborder[i],itemline[i],itemtrack[i],itemfill[i],trackborder[i]})do if d then toggle.setprop(d,"Transparency",(d.Transparency or 1)*contentfade)end end;local frame=sectionframes[i];if frame then for _,d in pairs(frame)do toggle.setprop(d,"Transparency",(d.Transparency or 1)*contentfade)end end end end
...
toggle.setinstacrate=function(value,quiet)
    local state=toggle.instacratestate;toggle.instacrate=value==true;state.nextapply=0;state.lastbox=nil;state.lastopen=0;local opened=toggle.instacrate and toggle.applyinstacrate(true)or false;menuupdate();if not quiet then bindlog(toggle.instacrate and"enabled insta-open crate"or"disabled insta-open crate")end;return opened
end
...
toggle.setguitextoutline=function(value,quiet)
    toggle.guitextoutline=value==true;local outlined=toggle.guitextoutline;for i=1,#toggle.huditems do toggle.huditems[i].value.Outline=outlined;toggle.huditems[i].label.Outline=outlined end;for i=1,#powerlines do powerlines[i].Outline=outlined end;for _,text in ipairs({powerlabel,toggle.powerempty,toggle.ppmsdraw.value,toggle.ppmsdraw.label})do text.Outline=outlined end;menuupdate();if not quiet then bindlog(outlined and"enabled GUI and HUD text outline"or"disabled GUI and HUD text outline")end
end
...
local function loadconfig(quiet)
    ...
    if tonumber(data.gui_outline_default_version)==2 and type(data.gui_text_outline)=="boolean"then toggle.setguitextoutline(data.gui_text_outline,true)else toggle.setguitextoutline(false,true)end;if type(data.esp_text_outline)=="boolean"then toggle.setesptextoutline(data.esp_text_outline,true)end
    ...
    if type(data.binds)=="table"then
        ...
        for i=1,#bindorder do local id=bindorder[i];local code=tonumber(data.binds[id]);if code==nil then code=keybinds[id]end;if id=="menu"and data.menu_default_version==nil and code==0xBB then code=defaultbinds.menu end;if legacydefaults and id~="menu"then code=0 end;if code==0 and id~="menu"then nextbinds[id]=0 elseif not keynames[code]or used[code]then valid=false else used[code]=true;nextbinds[id]=code end end
