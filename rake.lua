...
toggle.worldinfo={scraps=0,points=0,flare=false,traps=0,crates=0}
toggle.notifysettings={position="bottom right",duration=4,flare=true,crate=true,scrap=true,rakeclose=true,rakedistance=30}
toggle.notifications={items={},ready=false,rakenear=false,max=4}
toggle.scanid=0;toggle.scanning=false;toggle.nextpowerread=0
toggle.scrapvalues={12,15,19,23,27}
...
toggle.zoom={thirdperson=false,amount=10,min=0,max=0,originalmin=nil,originalmax=nil,memoryvalid=false,directvalid=false,nextapply=0,minoffset=0x35C,maxoffset=0x358}
toggle.shiftlockstate={offset=0x3B6,active=false,nextapply=0}
toggle.instacrate=false;toggle.instacratestate={nextapply=0,lastbox=nil,lastopen=0,range=25,unlockoffset=0xA8,patched=setmetatable({},{__mode="k"})}
toggle.shop={
...
local defaultbinds={menu=0x43,esp=0,hud=0,scrap=0,flare=0,aura=0,sell=0,thirdperson=0}
local keybinds={menu=defaultbinds.menu,esp=defaultbinds.esp,hud=defaultbinds.hud,scrap=defaultbinds.scrap,flare=defaultbinds.flare,aura=defaultbinds.aura,sell=defaultbinds.sell,thirdperson=defaultbinds.thirdperson}
local bindorder={"menu","esp","hud","scrap","flare","aura","sell","thirdperson"}
local bindlabels={menu="menu toggle",esp="esp toggle",hud="hud toggle",scrap="tp to scrap",flare="tp to flare",aura="stun aura",sell="tp sell scrap",thirdperson="third person"}
...
toggle.notify=function(text)
    local shown=tostring(text or"");local longest,lines=#shown,1;if toggle.wraptooltip then shown,longest,lines=toggle.wraptooltip(shown,38)end;local state=toggle.notifications;state.items[#state.items+1]={text=shown,width=math.max(190,math.min(306,longest*7+26)),height=math.max(50,lines*14+28),started=tick(),duration=toggle.notifysettings.duration,anim=0,x=nil,y=nil}
    while #state.items>state.max do table.remove(state.items,1)end
end
toggle.notifyevent=function(kind,text)if toggle.notifysettings[kind]then toggle.notify(text)end end
...
local menustate={w=392,h=470,watermarkw=100,watermarkh=27,x=24,y=math.floor((cam.ViewportSize.Y-470)/2),tab=1,minimized=false,minimizeanim=0,menuanim=0,contentfade=1,maxitems=96,dragsensitivity=1.5,scroll={},scrolltarget={},scrollmax={},itemkinds={},hover={},toggleanim={},slideranim={},tabfade=1,tabslide=0,indicatorx=nil,indicatorw=nil,itemsdirty=true}
...
toggle.menutitle=function()return"saint's menu"end
...
local itembg,itemlabel,itemvalue,itemmark,itemline,itemtrack,itemfill,itemborder,markborder,trackborder,itemarrow={},{},{},{},{},{},{},{},{},{},{}
...
for i=1,menustate.maxitems do
    itembg[i]=setz(newsquare(Color3.fromHex("#202330"),1),110);itemborder[i]=setz(newborder(Color3.fromHex("#272c35"),1),111);itemlabel[i]=setz(newtext("",Color3.fromHex("#ffffff"),false,false),121);itemvalue[i]=setz(newtext("",Color3.fromHex("#ffffff"),false,false),121);toggle.menuinfo.items[i]=setz(newtext("?",Color3.fromHex("#f2a93b"),false,false),122);itemmark[i]=setz(newsquare(Color3.fromHex("#7aa2f7"),1),116);markborder[i]=setz(newborder(Color3.fromHex("#343b46"),1),114);itemline[i]=setz(newline(Color3.fromHex("#787c99")),112);itemline[i].Thickness=1;itemarrow[i]=setz(Drawing.new("Triangle"),122);itemarrow[i].Filled=true;itemarrow[i].Thickness=1;itemarrow[i].Color=Color3.fromHex("#ffffff");itemarrow[i].Transparency=1;itemarrow[i].Visible=false;itemtrack[i]=setz(newsquare(Color3.fromHex("#343b58"),1),114);itemfill[i]=setz(newsquare(Color3.fromHex("#7aa2f7"),1),115);trackborder[i]=setz(newborder(Color3.fromHex("#343b46"),1),118);toggle.inlinecolors.mark[i]=setz(newsquare(Color3.fromHex("#ffffff"),1),116);toggle.inlinecolors.outer[i]=setz(newborder(Color3.fromHex("#000000"),1),114);toggle.inlinecolors.middle[i]=setz(newborder(Color3.fromHex("#343b46"),1),115);toggle.inlinecolors.inner[i]=setz(newborder(Color3.fromHex("#000000"),1),118)
end
local sectionframes={}
...
toggle.layoutitemshade=function(index,target,first)
    local shades=getitemrgb(index);local height=math.max(1,target.Size.Y);first=first or 1
    for n=0,3 do local top=math.floor(n*height/4);local bottom=math.floor((n+1)*height/4);local shade=shades[first+n];shade.Position=Vector2.new(target.Position.X,target.Position.Y+top);shade.Size=Vector2.new(target.Size.X,math.max(1,bottom-top))end
end
...
toggle.drawnotifications=function()
    ...
    for i=#state.items,1,-1 do local item=state.items[i];local open=now<item.started+item.duration;item.anim=toggle.ease(item.anim,open and 1 or 0,open and 0.24 or 0.18);if not open and item.anim<=0.01 then table.remove(state.items,i)end end
    ...
    for i=1,state.max do
        ...
        if item then
            ...
            local x,y=toggle.pixel(item.x),toggle.pixel(item.y);local alpha=item.anim;local shell=guiopacity*alpha;frame.outer.Position=Vector2.new(x,y);frame.outer.Size=Vector2.new(w,h);frame.middle.Position=Vector2.new(x+1,y+1);frame.middle.Size=Vector2.new(w-2,h-2);frame.inner.Position=Vector2.new(x+2,y+2);frame.inner.Size=Vector2.new(w-4,h-4);frame.bg.Position=Vector2.new(x+3,y+3);frame.bg.Size=Vector2.new(w-6,h-6);frame.top.Position=Vector2.new(x+3,y+3);frame.top.Size=Vector2.new(w-6,8);frame.text.Position=Vector2.new(x+13,y+17);frame.text.Text=item.text
            local ratio=clamp((item.started+item.duration-now)/math.max(0.01,item.duration),0,1);frame.progressbg.Position=Vector2.new(x+10,y+h-9);frame.progressbg.Size=Vector2.new(w-20,3);frame.progress.Position=frame.progressbg.Position;frame.progress.Size=Vector2.new(math.max(0,(w-20)*ratio),3);toggle.layoutgradient(frame.gradient,x+toggle.gradientinset(),y+3,math.max(1,w-toggle.gradientinset()*2))
            toggle.setprop(frame.outer,"Color",themes.borderblack);toggle.setprop(frame.middle,"Color",color("outline"));toggle.setprop(frame.inner,"Color",themes.borderblack);toggle.setprop(frame.bg,"Color",color("bg"));toggle.setprop(frame.top,"Color",color("top"));toggle.setprop(frame.text,"Color",color("text"));toggle.setprop(frame.progressbg,"Color",color("top"));toggle.setprop(frame.progress,"Color",color("accent"));toggle.setprop(frame.outer,"Transparency",shell);toggle.setprop(frame.middle,"Transparency",0.85*shell);toggle.setprop(frame.inner,"Transparency",shell);toggle.setprop(frame.bg,"Transparency",shell);toggle.setprop(frame.top,"Transparency",0.4*shell);toggle.setprop(frame.text,"Transparency",alpha);toggle.setprop(frame.progressbg,"Transparency",0.8*shell);toggle.setprop(frame.progress,"Transparency",alpha);toggle.applytextoutline(frame.text,alpha,color("text"));for _,d in ipairs({frame.outer,frame.middle,frame.inner,frame.bg,frame.top,frame.text,frame.progressbg,frame.progress})do toggle.setvisible(d,true)end;toggle.paintgradient(frame.gradient,true,shell)
        else
...
local function section(label,col,info,permission)return {kind="section",label=label,col=col or 1,info=info,permission=permission}end
...
toggle.itemshown=function(item)
    ...
    if item.kind=="dropdown"then if #shown>19 then shown=string.sub(shown,1,17)..".."end;return shown end
    if #shown>18 then shown=string.sub(shown,1,16)..".."end;return shown
end
toggle.dropdownkindof=function(id)
    return id=="espfontselect"and"espfont"or id=="hudfontselect"and"hudfont"or id=="presetselect"and"preset"or id=="distanceunitselect"and"unit"or id=="distancepositionselect"and"distanceposition"or id=="scrapstyleselect"and"scrapstyle"or id=="scrapteleportselect"and"scrapteleport"or id=="ringshapeselect"and"ringshape"or id=="rgbdirectionselect"and"rgbdirection"or id=="powerformatselect"and"powerformat"or id=="timerformatselect"and"timerformat"or id=="ppmsstyleselect"and"ppmsstyle"or id=="rakehealthformatselect"and"rakehealthformat"or id=="hudstyleselect"and"hudstyle"or id=="poweractivitymodeselect"and"poweractivitymode"or id=="notificationpositionselect"and"notificationposition"or id=="shopitemselect"and"shopitem"or id=="configselect"and"config"or nil
end
...
local function currentitems()
    ...
    if menustate.tab==1 then
        ...
        local items={
            ...
            section("world",2),{id="worldscraps",kind="info",label="scraps",value=tostring(toggle.worldinfo.scraps).." [+"..tostring(toggle.worldinfo.points).."p]",col=2},{id="worldflare",kind="info",label="flare spawned",value=toggle.worldinfo.flare and"yes"or"no",col=2},{id="worldtraps",kind="info",label="traps",value=tostring(toggle.worldinfo.traps),col=2},{id="worldcrates",kind="info",label="crates",value=tostring(toggle.worldinfo.crates),col=2},
            section("shop",1,"requires hybrid mode; shop actions teleport to the shop before buying, selling, or selling scrap","hybrid"),{id="shopitemselect",kind="dropdown",label="item",value=toggle.shoplabel(),col=1},{id="quickbuy",kind="action",label="quick buy",col=1},{id="quicksell",kind="action",label="quick sell",col=1},{id="sell",kind="toggle",label="tp sell scrap",on=toggle.sellenabled,bind="sell",inlinebind=true,col=1},
            section("teleports",2),{id="scrap",kind="toggle",label="tp to scrap",on=toggle.scrapteleportenabled,bind="scrap",inlinebind=true,col=2},{id="scrapteleportselect",kind="dropdown",label="sort by",value=toggle.scrapteleport,col=2},{id="flare",kind="toggle",label="tp to flare",on=toggle.flareteleportenabled,bind="flare",inlinebind=true,col=2},{id="teleportcooldown",kind="toggle",label="tp safe cooldown",on=toggle.teleportcooldown,info=teleportinfo,col=2},{id="cooldownseconds",kind="slider",label="cooldown time",value=toggle.cooldownseconds,min=10,max=60,display=tostring(toggle.cooldownseconds).."s",col=2},
            section("combat",1,"requires hybrid mode; stun aura doesn't use stamina and requires an equipped stun stick","hybrid"),{id="killaura",kind="toggle",label="stun aura",on=toggle.killaura,bind="aura",inlinebind=true,col=1},{id="killaurarange",kind="slider",label="stun range",value=toggle.killaurarange,min=6,max=30,display=tostring(toggle.killaurarange).." studs",col=1},{id="killauradelay",kind="slider",label="stun delay",value=toggle.killauradelay,min=0.05,max=0.6,display=string.format("%.2fs",toggle.killauradelay),col=1},
            section("camera",2,"requires unsafe LuaU mode; equip an item after toggling third person, and update shift lock through Roblox movement settings","unsafe"),{id="thirdperson",kind="toggle",label="third person",on=toggle.zoom.thirdperson,bind="thirdperson",inlinebind=true,col=2},{id="zoomamount",kind="slider",label="zoom amount",value=toggle.zoom.amount,min=0.5,max=100,display=string.format("%.1f studs",toggle.zoom.amount),col=2},{id="shiftlock",kind="toggle",label="shift lock",on=toggle.shiftlockstate.active,col=2},
            section("client",1),{id="noFall",kind="toggle",label="no fall damage",on=toggle.client.noFall,col=1},{id="noJumpCooldown",kind="toggle",label="no jump cooldown",on=toggle.client.noJumpCooldown,info="unstable: may need re-enabling and can freeze over time",warning=true,unstable=true,col=1},{id="infiniteStamina",kind="toggle",label="infinite stamina",on=toggle.client.infiniteStamina,info="unstable: may need re-enabling and can freeze over time",warning=true,unstable=true,col=1},
            section("supply crates",2),{id="supplylabel",kind="toggle",label="crate esp",on=toggle.supplylabel,colorindex=8,col=2},{id="notifycrate",kind="toggle",label="crate notify",on=toggle.notifysettings.crate,notifykey="crate",col=2},{id="supplyitems",kind="toggle",label="supplies esp",on=toggle.supplyitems,info="shows inventory of crate in order, also shows if item has been taken",col=2},{id="instacrate",kind="toggle",label="instant crate",on=false,disabled=true,info="requires hybrid mode, may not work with pre-existing crates",permission="hybrid",col=2},
            section("power usage",2),{id="hudpower",kind="toggle",label="power left",on=false,disabled=true,info="requires unsafe LuaU mode enabled",permission="unsafe",col=2},{id="poweractivity",kind="toggle",label="activity panel",on=toggle.poweractivity,col=2},{id="poweractivitymodeselect",kind="dropdown",label="show when",value=toggle.poweractivitymode,col=2},{id="ppms",kind="toggle",label="voltmeter level",on=toggle.ppms,col=2},
            ...
            section("rake esp",1),{id="rakename",kind="toggle",label="rake name",on=toggle.rakename,colorindex="rake",col=1},{id="rakenameinput",kind="text",stacked=true,label="custom name",value=toggle.rakenamecapture and toggle.rakenamevalue.."_"or toggle.rakenamevalue,col=1},{id="rakehealth",kind="toggle",label="rake health",on=toggle.rakehealth,colorindex="rakehealth",col=1},{id="rakedistance",kind="toggle",label="rake distance",on=toggle.rakedistance,col=1},{id="notifyrakeclose",kind="toggle",label="warn rake near",on=toggle.notifysettings.rakeclose,notifykey="rakeclose",col=1},{id="rakenotifydistance",kind="slider",label="warn distance",value=toggle.notifysettings.rakedistance,min=5,max=100,display=tostring(toggle.notifysettings.rakedistance).."m",col=1},{id="healthbased",kind="toggle",label="health-based color",on=toggle.healthbased,col=1},{id="rakehealthformatselect",kind="dropdown",label="health style",value=toggle.rakehealthformat,col=1},{id="rakenamey",kind="slider",label="name Y offset",value=toggle.rakenamey,min=-100,max=100,display=tostring(toggle.rakenamey).."px",col=1},{id="rakehealthy",kind="slider",label="health Y offset",value=toggle.rakehealthy,min=-100,max=100,display=tostring(toggle.rakehealthy).."px",col=1},{id="rakebarwidth",kind="slider",label="health bar width",value=toggle.rakebarwidth,min=30,max=200,display=tostring(toggle.rakebarwidth).."px",col=1},
            section("objects esp",2),{id="flares",kind="toggle",label="flare gun",on=espgroups.flares,colorindex=1,col=2},{id="notifyflare",kind="toggle",label="flare notify",on=toggle.notifysettings.flare,notifykey="flare",col=2},{id="traps",kind="toggle",label="rake trap",on=espgroups.traps,colorindex=7,col=2},
            section("scraps esp",1),{id="scrapstyleselect",kind="dropdown",label="scrap style",value=toggle.scrapstyle,col=1},{id="notifyscrap",kind="toggle",label="scrap notify",on=toggle.notifysettings.scrap,notifykey="scrap",col=1},{id="itemScrap1",kind="toggle",label="scrap 1",on=espgroups.items.Scrap1,itemkey="Scrap1",colorindex=2,col=1},{id="itemScrap2",kind="toggle",label="scrap 2",on=espgroups.items.Scrap2,itemkey="Scrap2",colorindex=3,col=1},{id="itemScrap3",kind="toggle",label="scrap 3",on=espgroups.items.Scrap3,itemkey="Scrap3",colorindex=4,col=1},{id="itemScrap4",kind="toggle",label="scrap 4",on=espgroups.items.Scrap4,itemkey="Scrap4",colorindex=5,col=1},{id="itemScrap5",kind="toggle",label="scrap 5",on=espgroups.items.Scrap5,itemkey="Scrap5",colorindex=6,col=1},
            section("timer",2),{id="hudtimer",kind="toggle",label="timer hud",on=toggle.hudelements.timer,col=2},{id="timerformatselect",kind="dropdown",label="timer style",value=toggle.timerformat,col=2}
        }
...
toggle.rowheight=function(item)return(item.stacked or item.kind=="dropdown")and 46 or item.kind=="section"and 22 or item.kind=="slider"and 35 or item.kind=="action"and 25 or 19 end
...
local function menupos()
    ...
    for i=1,#menuitems do
        local item=menuitems[i];local col=item.col or 1;local x=left+(col-1)*(w+gap)+math.floor(menustate.tabslide+0.5);local y=ys[col];local h=toggle.rowheight(item);local panelh=groupheights[i]or h;local stacked=item.stacked or item.kind=="dropdown"
        ...
        local texty=y+(item.kind=="section"and 8 or item.kind=="action"and 12 or item.kind=="dropdown"and 3 or item.kind=="toggle"and 3 or 4);local valuey=item.kind=="dropdown"and y+24 or stacked and y+27 or texty;local renderh=item.kind=="section"and panelh or h
        local overlap=math.min(y+renderh,clipbottom)-math.max(y,cliptop);local visible=overlap>0;local fade=(visible and clamp(math.min(y+renderh-cliptop,clipbottom-y)/14,0,1)or 0)*menustate.tabfade;local fieldtop=stacked and y+18 or y;local fieldbottom=y+h;local fieldoverlap=math.min(fieldbottom,clipbottom)-math.max(fieldtop,cliptop);local fieldvisible=fieldoverlap>0;local fieldfade=(fieldvisible and clamp(math.min(fieldbottom-cliptop,clipbottom-fieldtop)/14,0,1)or 0)*menustate.tabfade
        local textfade=(visible and math.min(renderscroll>0 and clamp((texty-cliptop)/12,0,1)or 1,renderscroll<menustate.scrollmax[menustate.tab]and clamp((clipbottom-texty-10)/12,0,1)or 1)or 0)*menustate.tabfade;local valuefade=stacked and(visible and math.min(renderscroll>0 and clamp((valuey-cliptop)/12,0,1)or 1,renderscroll<menustate.scrollmax[menustate.tab]and clamp((clipbottom-valuey-10)/12,0,1)or 1)or 0)*menustate.tabfade or textfade
        ...
        if visible then
        if item.kind~="section"then local narrow=item.kind=="action"or item.kind=="text"or item.kind=="dropdown";local fieldx=narrow and x+7 or x;local fieldw=narrow and w-14 or w;itembg[i].Position=Vector2.new(fieldx,stacked and y+18 or y);itembg[i].Size=Vector2.new(fieldw,stacked and(item.kind=="dropdown"and 24 or 27)or h-1)end;itemlabel[i].Position=Vector2.new(x+(item.kind=="section"and 12 or item.kind=="toggle"and 23 or 7),texty);itemvalue[i].Text=shown;itemvalue[i].Position=Vector2.new(stacked and x+15 or x+w-7-math.floor(#shown*7),valuey)
        if item.kind=="dropdown"then local ax,ay=x+w-18,y+30;local opened=toggle.dropdownkindof(item.id)==dropdownkind;if opened then itemarrow[i].PointA=Vector2.new(ax-2,ay+3);itemarrow[i].PointB=Vector2.new(ax-2,ay-3);itemarrow[i].PointC=Vector2.new(ax+3,ay)else itemarrow[i].PointA=Vector2.new(ax-4,ay-2);itemarrow[i].PointB=Vector2.new(ax+4,ay-2);itemarrow[i].PointC=Vector2.new(ax,ay+3)end end
        itemlabel[i].Center=item.kind=="action";toggle.menuinfo.items[i].Center=item.kind=="action"and item.info~=nil;if item.kind=="section"and item.info then local infox=x+17+#item.label*7;toggle.menuinfo.items[i].Position=Vector2.new(infox,texty);toggle.menuinfo.layouts[i]={x=infox-2,y=texty-2,w=25,h=14,text=item.info,warning=item.warning,unstable=item.unstable,permission=item.permission,visible=visible and texty>=cliptop and texty+10<=clipbottom}elseif item.kind=="action"then if item.info then local labelw=#item.label*7;local totalw=labelw+28;local startx=x+math.floor((w-totalw)/2);itemlabel[i].Position=Vector2.new(startx+labelw/2,texty);toggle.menuinfo.items[i].Position=Vector2.new(startx+labelw+17.5,texty);toggle.menuinfo.layouts[i]={x=startx+labelw+5,y=texty-2,w=25,h=14,text=item.info,warning=item.warning,unstable=item.unstable,permission=item.permission,visible=visible and texty>=cliptop and texty+10<=clipbottom}else itemlabel[i].Position=Vector2.new(x+w/2,texty)end elseif item.info then local infox=x+w-(item.colorindex and 57 or item.inlinebind and(#shown*7+35)or 24);toggle.menuinfo.items[i].Position=Vector2.new(infox,texty);toggle.menuinfo.layouts[i]={x=infox-2,y=texty-2,w=25,h=14,text=item.info,warning=item.warning,unstable=item.unstable,permission=item.permission,visible=visible and texty>=cliptop and texty+10<=clipbottom}end
        ...
        local markx,marky,markw,markh=item.kind=="toggle"and x+7 or x+w-16,y+4,9,9;if item.kind=="color"then markx,markw=x+w-25,18 end
        itemmark[i].Position=Vector2.new(markx+1,marky+1);itemmark[i].Size=Vector2.new(markw-2,markh-2)
        if item.colorindex then itemlayouts[i].colorx=x+w-27;itemlayouts[i].colory=marky-2;itemlayouts[i].colorw=22;itemlayouts[i].colorh=13;toggle.inlinecolors.mark[i].Position=Vector2.new(x+w-24,marky+1);toggle.inlinecolors.mark[i].Size=Vector2.new(16,7);toggle.inlinecolors.outer[i].Position=Vector2.new(x+w-27,marky-2);toggle.inlinecolors.outer[i].Size=Vector2.new(22,13);toggle.inlinecolors.middle[i].Position=Vector2.new(x+w-26,marky-1);toggle.inlinecolors.middle[i].Size=Vector2.new(20,11);toggle.inlinecolors.inner[i].Position=Vector2.new(x+w-25,marky);toggle.inlinecolors.inner[i].Size=Vector2.new(18,9)end
        ...
        if item.kind=="section"then
            local bgtop=math.max(cliptop,y+17);local bgbottom=math.min(clipbottom,y+panelh-4);itembg[i].Position=Vector2.new(x-2,bgtop);itembg[i].Size=Vector2.new(w+4,math.max(0,bgbottom-bgtop));local frame=getsectionframe(i);local titleend=math.min(x+w-12,x+12+#item.label*7+(item.info and 24 or 0))
...
local function menuobjects(visible)
    ...
    for i=1,itemcount do
        local item=menuitems[i];local layout=itemlayouts[i];local on=expanded and item~=nil and layout and layout.visible;local sectionon=on and item.kind=="section";local slideron=on and item.kind=="slider";local markon=on and(item.kind=="toggle"or item.kind=="color");local markcfg=markon and item.kind=="color"and entrycfg(item.index)or nil;local rgbmarkon=false;local infoon=on and item.kind=="info";local fieldon=on and item.kind~="info"and(not(item.stacked or item.kind=="dropdown")or layout.fieldvisible);local bordered=fieldon and(item.kind=="action"or item.kind=="text"or item.kind=="dropdown");local inlineon=on and item.colorindex~=nil and layout.markvisible
        local sliderframeon=slideron and layout.trackvisible;local markframeon=markon and layout.markvisible
        toggle.setvisible(itembg[i],item and item.kind=="section"and on or fieldon);toggle.setvisible(itemborder[i],sliderframeon or markframeon or bordered);toggle.setvisible(itemlabel[i],on and layout.textvisible);toggle.setvisible(toggle.menuinfo.items[i],on and item.info~=nil and layout.textvisible);toggle.setvisible(itemvalue[i],(fieldon or infoon)and layout.valuevisible and item.kind~="section"and(item.kind~="toggle"or item.inlinebind)and item.kind~="color");toggle.setvisible(itemmark[i],markframeon and not rgbmarkon);toggle.setvisible(markborder[i],sliderframeon or markframeon or bordered);toggle.setvisible(itemline[i],false);toggle.setvisible(itemarrow[i],on and item.kind=="dropdown"and layout.valuevisible);toggle.setvisible(itemtrack[i],sliderframeon);toggle.setvisible(itemfill[i],sliderframeon);toggle.setvisible(trackborder[i],sliderframeon or markframeon or bordered)
        ...
        if itemrgb[i]then local mainshade=sliderframeon or markframeon and(item.kind=="color"or item.on);for s=1,itemrgbcount do toggle.setvisible(itemrgb[i][s],s<=4 and mainshade or s>4 and inlineon)end end
    end
...
local function menuupdate(animateonly)
    ...
    for i=1,#menuitems do
        ...
        if itemrgb[i]then local mainalpha=item.kind=="slider"and borderfade or item.kind=="toggle"and(menustate.toggleanim[animkey]or 0)*fade or fade;for s=1,itemrgbcount do local localstep=(s-1)%4+1;toggle.setprop(itemrgb[i][s],"Color",themes.borderblack);toggle.setprop(itemrgb[i][s],"Transparency",(0.025+localstep*0.035)*(s<=4 and mainalpha or fade))end end
...
toggle.applyradius=function()
    ...
    for _,frame in ipairs(toggle.toastframes)do for _,entry in ipairs({{frame.outer,0},{frame.middle,1},{frame.inner,2},{frame.bg,3},{frame.top,3}})do pcall(function()entry[1].Corner=math.max(0,toggle.borderradius-entry[2])end)end;pcall(function()frame.progressbg.Corner=math.min(2,toggle.borderradius);frame.progress.Corner=math.min(2,toggle.borderradius)end)end
...
local function powerpos()
    ...
    local showalways=toggle.poweractivitymode=="always";local hasactivity=n>0;local empty=toggle.poweractivity and showalways and not hasactivity;local rows=toggle.poweractivity and(empty and 1 or n)or 0;local volt=toggle.ppms and not toggle.ppmspowerblocked and(hasactivity or showalways);local shown=math.min(toggle.ppmslevel or 0,5);local contentw=toggle.textwidth(powerlabel);if empty then contentw=math.max(contentw,toggle.textwidth(toggle.powerempty))end;for i=1,#powerlines do if p.lineactive[i]then contentw=math.max(contentw,toggle.textwidth(powerlines[i]))end end;if volt and shown>0 then contentw=math.max(contentw,shown*12+(shown-1)*3)end;p.w=math.max(106,contentw+26);p.h=40+rows*17+(volt and 23 or 0);if not p.dragged then p.x=cam.ViewportSize.X-p.w-18;p.y=math.floor((cam.ViewportSize.Y-p.h)/2)end;p.x=clamp(p.x,0,math.max(0,cam.ViewportSize.X-p.w));p.y=clamp(p.y,0,math.max(0,cam.ViewportSize.Y-p.h));local activityon=toggle.poweractivity and(hasactivity or showalways);local on=toggle.hud and(activityon or volt);p.anim=toggle.ease(p.anim,on and 1 or 0,on and 0.26 or 0.3);local drawon=p.anim>0.01;local drawy=p.y+math.floor((1-p.anim)*8+0.5);local alpha=guiopacity*p.anim;local contentalpha=p.anim
    ...
    n=0;for i=1,#powerlines do local line=powerlines[i];local lineon=drawon and toggle.poweractivity and p.lineactive[i]or false;toggle.setvisible(line,lineon);if lineon then n=n+1;if geometrychanged then line.Position=Vector2.new(p.x+13,drawy+31+(n-1)*17)end;toggle.setprop(line,"Color",color("text"));toggle.setprop(line,"Transparency",contentalpha);toggle.applytextoutline(line,contentalpha)end end
    if geometrychanged then toggle.powerempty.Position=Vector2.new(p.x+13,drawy+31)end;toggle.setprop(toggle.powerempty,"Color",color("muted"));toggle.setprop(toggle.powerempty,"Transparency",contentalpha);toggle.applytextoutline(toggle.powerempty,contentalpha);toggle.setvisible(toggle.powerempty,drawon and empty)
    ...
    local total=shown>0 and shown*12+(shown-1)*3 or 0;local startx=p.x+math.floor((p.w-total)/2);local celly=drawy+31+rows*17
...
local function track(inst)
    ...
    local existing=toggle.bysource[inst];if existing then existing.seen=toggle.scanid;return end
    ...
    existing=byaddr[address];if existing then existing.seen=toggle.scanid;return end
    ...
    if toggle.notifications.ready then local tier=tonumber(string.match(cfgname or"","^Scrap(%d+)$"));if tier then toggle.notifyevent("scrap","scrap "..tostring(tier).." spawned [+"..tostring(toggle.scrapvalues[tier]or 0).."p]")elseif cfgname=="FlareGunPickUp"then toggle.notifyevent("flare","flare gun spawned")elseif cfg.group=="crates"then toggle.notifyevent("crate","supply crate spawned")end end
end
...
local function scan()
    ...
    for i=1,#tracked do local rec=tracked[i];rec.active=rec.seen==toggle.scanid and rec.object~=nil and rec.object.Parent~=nil end
    local scraps,points,flare,traps,crates=0,0,false,0,0
    for i=1,#tracked do local rec=tracked[i];if rec.active then local tier=rec.cfgname and tonumber(string.match(rec.cfgname,"^Scrap(%d+)$"));if tier then scraps=scraps+1;points=points+(toggle.scrapvalues[tier]or 0)elseif rec.cfgname=="FlareGunPickUp"then flare=true elseif rec.cfg.group=="traps"then traps=traps+1 elseif rec.cfg.group=="crates"then crates=crates+1 end end end
    local info=toggle.worldinfo;if info.scraps~=scraps or info.points~=points or info.flare~=flare or info.traps~=traps or info.crates~=crates then info.scraps,info.points,info.flare,info.traps,info.crates=scraps,points,flare,traps,crates;if toggle.menu and menustate.tab==1 then for i=1,#menuitems do local item=menuitems[i];if item.id=="worldscraps"then item.value=tostring(scraps).." [+"..tostring(points).."p]"elseif item.id=="worldflare"then item.value=flare and"yes"or"no"elseif item.id=="worldtraps"then item.value=tostring(traps)elseif item.id=="worldcrates"then item.value=tostring(crates)end;if item.kind=="info"and itemvalue[i]then itemvalue[i].Text=toggle.itemshown(item)end end end end;toggle.notifications.ready=true
end
...
local function drawcrate(rec,screen,meters,yoffset)
    ...
    local now=toggle.frametime or tick();if not rec.itemscache or now-(rec.itemcachetime or 0)>=0.05 then rec.itemscache=rec.folder:GetChildren();rec.itemcachetime=now end;local children=rec.itemscache
...
local function rakeinfo()
    ...
    local state=toggle.notifications;local present=toggle.rakepart~=nil and toggle.rakepart.Parent~=nil;if present and toggle.notifysettings.rakeclose then local meters=dist(viewpos(),toggle.rakepart.Position);if meters<=toggle.notifysettings.rakedistance and not state.rakenear then state.rakenear=true;toggle.notifyevent("rakeclose","rake is nearby ["..tostring(math.floor(meters)).."m]")elseif meters>toggle.notifysettings.rakedistance+5 then state.rakenear=false end else state.rakenear=false end
...
local function scraphud()
    ...
    local now=tick();if toggle.hudelements.power and now>=toggle.nextpowerread then
        toggle.nextpowerread=now+0.1;if not toggle.powerlevel or not toggle.powerlevel.Parent then toggle.powerlevel=powervals:FindFirstChild("PowerLevel")end
        local level=toggle.powerlevel;local ok,minimum,maximum,value=pcall(function()
            local address=level and level.Address
            if type(address)~="number"or address<=0 then return nil end
            return memory_read("uintptr_t",address+0xC0),memory_read("uintptr_t",address+0xB8),memory_read("uintptr_t",address+0xA8)
        end)
        if ok and type(minimum)=="number"and type(maximum)=="number"and type(value)=="number"and maximum>minimum and value>=minimum and value<=maximum then toggle.poweravailable=value>0;local percent=(value-minimum)/(maximum-minimum)*100;toggle.powerdraw.value.Text=string.format("%.1f%%",percent)else toggle.powerdraw.value.Text="?";toggle.poweravailable=true end
    end
    return oldscrap~=scraptxt.Text or oldpower~=toggle.powerdraw.value.Text
end
local function timerhud()
    if toggle.uibatch then toggle.huddirty=true;return false end
    local changed=false;local timer=math.max(0,math.floor(tonumber(timerval.Value)or 0));local shown=toggle.timerformat=="seconds"and tostring(timer)or string.format("%d:%02d",math.floor(timer/60),timer%60);if timertxt.Text~=shown then timertxt.Text=shown;changed=true end
...
local function drawrgb()
    ...
    powerpos();toggle.drawnotifications()
end
...
toggle.shopaction=function(action)
    ...
    local moved=pcall(function()root.CFrame=destination.CFrame+Vector3.new(0,3,0)end);if not moved then return false,"shop"end;toggle.startcooldown();task.wait(0.1);local fired=true
...
toggle.setinstacrate=function(value,quiet)
    local state=toggle.instacratestate;toggle.instacrate=false;state.nextapply=0;state.lastbox=nil;state.lastopen=0;menuupdate();if not quiet then bindlog("instant crate is unavailable")end;return false
end
toggle.sethudelement=function(id,value,quiet)
    if toggle.hudelements[id]==nil then return end;if id=="power"then value=false end;toggle.hudelements[id]=value==true;hudpos();showhud();menuupdate();if not quiet then bindlog((toggle.hudelements[id]and"enabled "or"disabled ")..id.." HUD")end
end
...
toggle.setcooldownseconds=function(value,quiet)
    toggle.cooldownseconds=math.floor(clamp(tonumber(value)or toggle.cooldownseconds,10,60)+0.5);menuupdate();if not quiet then bindlog("tp safe cooldown set to "..tostring(toggle.cooldownseconds).."s")end
end
...
toggle.themepalette=function(selected)
    ...
    for _,cfg in pairs(toggle.hudstyles)do put(cfg,text)end;for _,cfg in pairs(toggle.hudvalues)do put(cfg,text)end
...
local function loadconfig(quiet)
    ...
    toggle.uibatch=true
    toggle.hybridmode=true;toggle.unsafeluau=true
    ...
    toggle.setcooldownseconds(type(data.teleport_cooldown_seconds)=="number"and data.teleport_cooldown_seconds or 30,true);toggle.setppmsstyle("voltmeter",true);toggle.setppmssquares(5,true)
    if type(data.notifications)=="table"then local n=data.notifications;toggle.setnotificationposition(n.position,true);toggle.setnotificationduration(n.duration,true);toggle.setrakenotifydistance(n.rakedistance,true);for _,id in ipairs({"flare","crate","scrap","rakeclose"})do if type(n[id])=="boolean"then toggle.notifysettings[id]=n[id]end end end
...
toggle.resettheme=function(quiet)
    setthemeindex(toggle.defaultthemeindex,true);setguiopacity(0.9,true);toggle.setborderradius(10,true);menuupdate();if not quiet then bindlog("reset theme")end
end
toggle.resettoggles=function(quiet)
    setesp(true,true);sethud(true,true);for _,id in ipairs({"timer","target","scrap","power"})do toggle.sethudelement(id,true,true)end;toggle.watermark=true;toggle.hybridmode=true;toggle.unsafeluau=true;toggle.hudlabels=true;toggle.sethudstyle("container",true);toggle.setguitextoutline(true,true);toggle.setesptextoutline(true,true);toggle.widgetgroup.x=nil;toggle.widgetgroup.y=nil;toggle.widgetgroup.dragged=false;toggle.distanceminimum=false;toggle.distancemin=20;toggle.healthbased=false;toggle.setpoweractivity(true,true);toggle.setpoweractivitymode("activity",true);toggle.cooldownuntil=0;toggle.cooldownremaining=0;toggle.setteleportcooldown(false,true);toggle.setcooldownseconds(30,true);toggle.setnotificationposition("bottom right",true);toggle.setnotificationduration(4,true);toggle.setrakenotifydistance(30,true);for _,id in ipairs({"flare","crate","scrap","rakeclose"})do toggle.notifysettings[id]=true end;toggle.setppms(false,true);toggle.setppmsstyle("voltmeter",true);toggle.setppmssquares(5,true);toggle.setroof(true,true);toggle.setrakename(true,true);toggle.setrakehealth(true,true);toggle.setrakedistance(false,true);toggle.rakenamevalue="rake";toggle.rakedraw.name.Text="rake";toggle.setrakenamey(0,true);toggle.setrakehealthy(0,true);toggle.setrakehealthformat("value",true);toggle.setrakebarwidth(70,true);setbarrgb(true,true);toggle.setrgbdirection("left",true);toggle.setrgbspeed(0.6,true);setdistance(false,true);toggle.distancefade=false;toggle.setunit("meters",true);toggle.setdistanceposition("below",true);toggle.setscrapstyle("default",true);toggle.setscrapteleport("value",true);toggle.setsupplylabel(true,true);toggle.setsupplyitems(true,true);toggle.setringenabled(true,true);toggle.setringshape("circle",true);toggle.setringfade(40,true);toggle.setringsize(1,true);toggle.setringspin(false,true);toggle.setringspinspeed(1,true);toggle.setpowerformat("percent",true);toggle.setpowerdecimal(true,true);toggle.settimerformat("clock",true)
...
toggle.resetbinds=function(quiet)
    keybinds={menu=defaultbinds.menu,esp=defaultbinds.esp,hud=defaultbinds.hud,scrap=defaultbinds.scrap,flare=defaultbinds.flare,aura=defaultbinds.aura,sell=defaultbinds.sell,thirdperson=defaultbinds.thirdperson};capture=nil;menuupdate();if not quiet then bindlog("reset binds")end
end
local function resetsettings()
    ...
    local v=cam.ViewportSize;menustate.x=24;menustate.y=math.floor((v.Y-menustate.h)/2);menustate.minimized=false;toggle.powerpanel.dragged=false;toggle.powerpanel.x=math.max(2,v.X-182);toggle.powerpanel.y=math.max(2,math.floor(v.Y/2-toggle.powerpanel.h/2));capture=nil;pickerentry=nil;picker.hexactive=false;dropdownkind=nil;configcapture=false;toggle.rakenamecapture=false
...
local function mouseinput()
    ...
    if not down then
        if inputstate.sliding=="zoomamount"then bindlog("zoom amount set to "..string.format("%.1f",toggle.zoom.amount).." studs")elseif inputstate.sliding=="fontsize"then bindlog("font size set to "..tostring(espfontsize))elseif inputstate.sliding=="killaurarange"then bindlog("stun aura range set to "..tostring(toggle.killaurarange).." studs")elseif inputstate.sliding=="killauradelay"then bindlog("stun aura delay set to "..string.format("%.2fs",toggle.killauradelay))elseif inputstate.sliding=="ringfade"then bindlog("ring distance updated")elseif inputstate.sliding=="ringsize"then bindlog("ring size set to "..string.format("%.1fx",toggle.ringsize))elseif inputstate.sliding=="ringspinspeed"then bindlog("ring spin speed updated")elseif inputstate.sliding=="rgbspeed"then bindlog("rainbow speed updated")elseif inputstate.sliding=="cooldownseconds"then bindlog("tp safe cooldown set to "..tostring(toggle.cooldownseconds).."s")elseif inputstate.sliding=="notificationduration"then bindlog("notification time set to "..string.format("%.1fs",toggle.notifysettings.duration))elseif inputstate.sliding=="rakenotifydistance"then bindlog("rake warning distance set to "..tostring(toggle.notifysettings.rakedistance).."m")elseif inputstate.sliding=="rakenamey"then bindlog("rake name Y offset set to "..tostring(toggle.rakenamey).."px")elseif inputstate.sliding=="rakehealthy"then bindlog("rake health Y offset set to "..tostring(toggle.rakehealthy).."px")elseif inputstate.sliding=="rakebarwidth"then bindlog("rake health bar width set to "..tostring(toggle.rakebarwidth).."px")elseif inputstate.sliding=="opacity"then bindlog("GUI opacity set to "..tostring(math.floor(guiopacity*100+0.5)).."%")elseif inputstate.sliding=="borderradius"then bindlog("border radius set to "..tostring(toggle.borderradius).."px")elseif(inputstate.sliding=="pickersquare"or inputstate.sliding=="pickerhue")and pickerentry then bindlog("updated "..toggle.colorname(pickerentry).." color")end
...
toggle.drawesp=function()
    ...
    while i>=1 do local rec=tracked[i];local remove=not toggle.scanning and rec.active==false;if not remove then local ok,alive=pcall(drawrec,rec,viewer);if ok then rec.drawfail=0;remove=not toggle.scanning and not alive elseif not rec.object or not rec.object.Parent then remove=not toggle.scanning else rec.drawfail=(rec.drawfail or 0)+1 end end;if remove then untrack(i)end;i=i-1 end
...
spawn(function()
    local nextrake=0
    while true do
        toggle.scanning=true;pcall(scan);toggle.scanning=false;local now=tick();if now>=nextrake then pcall(rakeinfo);nextrake=now+1 end
        task.wait(scanrate)
    end
end)
spawn(function()
