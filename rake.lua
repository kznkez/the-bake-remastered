...
local lp=Players.LocalPlayer; local vs="1.5.0"
...
local targetTitle=T{Center=true,Size=15,Color=Color3.fromHex("#ff8d8d"),Text="RAKE'S TARGET",Visible=true}
local timerLabel=T{Center=true,Size=15,Color=Color3.fromHex("#eeeeee"),Text="TIME LEFT",Visible=true}
local ppmsLabel=T{Center=true,Size=15,Color=Color3.fromHex("#ffe1b9"),Text="POWER USE",Visible=true}
local powerTitle=T{Center=false,Size=24,Color=Color3.fromHex("#ffe1b9"),Text="POWER ACTIVITY",Visible=false}
local powerLines,POWER_LINE_H={},18
for k,v in pairs(pwrStatMap) do powerLines[k]=T{Center=false,Size=15,Color=Color3.fromHex("#ffffff"),Text=v,Visible=false} end

local radioTitle=T{Center=false,Size=18,Color=Color3.fromHex("#cbffcf"),Text="RADIO DISPLAY",Visible=true}
local radLine={}; local LINE_H=20
for i=1,7 do radLine[i]={name=T{Center=false,Size=13,Color=Color3.fromHex("#dadada"),Text="",Visible=true},msg=T{Center=false,Size=13,Color=Color3.fromHex("#ffffff"),Text="",Visible=true}} end
...
local espObj={
    ...
    SupplyCrate={Type="Model",Root="PlacementPart",Text="Crate",Color=Color3.fromHex("#a09dff")}
}

local function fmt(s) s=math.max(0,math.floor(s)); return ("%d:%02d"):format(math.floor(s/60),s%60) end

local function addObj(v)
    local d=espObj[v.Name]; if not d or tempObj[v.Address] or not v:IsA(d.Type) then return end
    local r
    if d.Type=="BasePart" then r=v
    elseif v.Name:find("Scrap") then for _,c in pairs(v:GetDescendants()) do if c:IsA("BasePart") and c.Name==d.Root then r=c; break end end
    else r=v:FindFirstChild(d.Root) end
    if not r then return end
    local name=newText{Text=d.Text,Color=d.Color,Outline=true,Center=true,Size=14,Font=FONT,Visible=false}
    local dist=newText{Text="0 studs",Color=Color3.fromHex("#969696"),Outline=true,Center=true,Size=12,Font=FONT,Visible=false}
    tempObj[v.Address]=true; tList[#tList+1]={object=r,model=v,name=name,dist=dist,Address=v.Address}
end
...
local function updPos()
    ...
    for i=#tList,1,-1 do local v=tList[i]; local o=v.object if not o or not o.Parent then if v.name then v.name:Remove() end; if v.dist then v.dist:Remove() end; tList[i]=tList[#tList]; tList[#tList]=nil; tempObj[v.Address]=nil else local pos3=o.Position; local screenPos,onScreen=WorldToScreen(pos3) if onScreen then local studsDist=0 if rx then studsDist=math.sqrt((pos3.X-rx)^2+(pos3.Y-ry)^2+(pos3.Z-rz)^2) end local studs=math.floor(studsDist) local yOffset=(v.model and espObj[v.model.Name] and espObj[v.model.Name].offY) or 0 v.name.Position=Vector2.new(screenPos.X,screenPos.Y-12+yOffset); v.name.Visible=true v.dist.Position=Vector2.new(screenPos.X,screenPos.Y+2+yOffset); v.dist.Text=studs.." studs"; v.dist.Visible=true else v.name.Visible=false; v.dist.Visible=false end end end
end
...
spawn(function()
    while true do
        ...
        powerTitle.Visible=toggle.hud and active>0; updPowerHudPos()

        local hrp=TargetVal.Value
        if hrp and hrp.ClassName=="Part" then local char=getCharacterFromPart(hrp); targetPlayerText.Text=char and char.Name or "Unknown" else targetPlayerText.Text="None" end
